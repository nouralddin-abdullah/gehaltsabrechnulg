...
test('sumBrutto: only GB=J rows are summed', () => {
  const rows = [
    { gb: 'J', menge: '79,75', faktor: '20,00' },           // 1595
    { gb: 'J', menge: '8',     faktor: '20,00' },           //  160
    { gb: 'J', menge: '90,50', faktor: '20,00' },           // 1810
    { gb: 'N', menge: '100',   faktor: '1,00' },            // excluded
    { hinweis: true, text: 'Nachberechnung 09/2023' },      // excluded
  ];
...
.slip .footer-row .cell .label { font-size: var(--caption-size); display: block; }
.slip .footer-row .cell .value { font-family: var(--mono); font-size: var(--value-size); }
...
.slip .footer-row .auszahlung .label { font-size: var(--caption-size); display: block; text-align: left; }
.slip .footer-row .auszahlung .value { font-family: var(--mono); font-size: 11pt; font-weight: 700; }
...
<div class="footnotes">
  ...
  <div>² Std = Stunden, T = Tage, Km = Kilometer, St = Stück; EUR = Euro, Tsd = Tausend Euro, Mio = Million Euro</div>
  <div>³ Gegebenenfalls Netto-Lohn/Netto-Stundenlohn</div>
  <div>⁴ L = Laufender Bezug, S = Sonstiger Bezug, F = Frei, E = Einmalbezug, P = Pauschalierung, A = Abfindung, M = mehrjährige Versteuerung, N = Nachberechnung, V = Vorjahr, W = Entgeltguthaben</div>
...
<div class="legal-line">- Dies ist eine Entgeltbescheinigung nach § 108 Abs. 3 Satz 1 der Gewerbeordnung -</div>
...
function renderFooter() {
  ...
  const t = computeTotals(state);
  document.getElementById('fAuszahlung').textContent = formatDE(t.auszahlungsbetrag);
}
```
...
.form-panel textarea { min-height: 50px; resize: vertical; }
.form-panel .toolbar {
  display: flex;
  gap: 6px;
  flex-wrap: wrap;
...
.form-panel button.secondary { background: #6c757d; }
.form-panel button.danger { background: #c82333; }
...
.form-panel .row-editor input { font-size: 11px; padding: 3px 5px; }
...
<section class="form-panel" aria-label="Eingabe">
  ...
  <h2>Firma</h2>
  <label>Eine Zeile, mit * als Trenner<textarea data-bind="firma"></textarea></label>
  ...
  <div class="grid">
    <label><input type="checkbox" data-bind-check="verdienst.nachberechnungVorjahr"> mit Nachberechnung Vorjahr</label>
    <span></span>
    ...
    <label>Steuerfreie Bezüge<input data-bind="verdienst.steuerfreieBezuege"></label>
    <label>VWL gesamt<input data-bind="verdienst.vwlGesamt"></label>
    <label>P. verst. Zuk.sich.<input data-bind="verdienst.pVerstZukSich"></label>
  ...
  state.brutto.forEach((r, i) => {
    ...
    if (r.hinweis) {
      ...
      wrap.querySelector('input').addEventListener('input', e => {
        r.text = e.target.value; render();
      });
    } else {
      wrap.className = 'row-editor';
      const fields = ['lohnart','bezeichnung','einheit','menge','faktor','prozent','st','sv','gb'];
      const placeholders = ['Lohnart','Bezeichnung','Einheit','Menge','Faktor','%','St','SV','GB'];
      let html = '';
    ...
    wrap.querySelector('[data-action="del"]').addEventListener('click', () => {
      state.brutto.splice(i, 1); render();
    });
...
function renderEditors() {
  renderBruttoEditor();
  renderRowsEditor('steuerEditor', state.steuer,
    ['tag','steuerBrutto','lohnsteuer','kirchensteuer','soli'],
    ['Tag (L/N)','Steuer-Brutto','Lohnsteuer','Kirchensteuer','Soli']);
  renderRowsEditor('svEditor', state.sv,
    ['tag','kvBrutto','rvBrutto','avBrutto','pvBrutto','kvBeitrag','rvBeitrag','avBeitrag','pvBeitrag'],
    ['Tag','KV-Bru','RV-Bru','AV-Bru','PV-Bru','KV-Bei','RV-Bei','AV-Bei','PV-Bei']);
  renderRowsEditor('nettoEditor', state.nettoBezuege,
...
function wireToolbar() {
  document.getElementById('btnAddBrutto').addEventListener('click', () => {
    state.brutto.push({ lohnart:'', bezeichnung:'', einheit:'Std', menge:'', faktor:'', prozent:'', st:'L', sv:'L', gb:'J' });
    render();
  });
